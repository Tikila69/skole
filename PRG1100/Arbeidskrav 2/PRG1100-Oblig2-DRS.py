...
def studentredigeringWindow3():
    ...
    entFornavn = Entry(window3, width=30, state="readonly",
                       textvariable=fornavn)
...
def studentslettingWindow4():
    ...
    entFornavn = Entry(window4, width=30, state="readonly",
                       textvariable=fornavn)
    ...
    entEtternavn = Entry(
        window4, width=20, state="readonly", textvariable=etternavn)
    ...
    # knapper
    btnSlett = Button(window4, text="Slett valgt student",
                      command=slettStudent)
...
def eksamensregistreringWindow5():
    ...
    listeRom = Listbox(window5, width=20, height=10,
                       listvariable=romlisteInnhold, yscrollcommand=yScroll.set)
    ...
    btnFinnLedigRom = Button(
        window5, text="Finn ledig rom", command=finnLedigRom)
    ...
    entAntallPlasser = Entry(
        window5, width=3, state="readonly", textvariable=antallPlasser)
...
def eksamensredigeringWindow6():
    ...
    def lagreEndringer():
        ...
        if duplikat == False:
            oppdatering = (
                "UPDATE Eksamen SET Romnr=%s WHERE Emnekode=%s AND Dato=%s")
    ...
    def hentEksamensinfo(event):
        ...
        seleksjonEksamen = (
            "SELECT * FROM Eksamen WHERE Dato=%s AND Emnekode=%s AND Romnr=%s")
    ...
    yScroll.grid(row=0, column=2, rowspan=10, padx=(0, 100), pady=5, sticky=NS)
    eksamensliste = Listbox(window6, width=25, height=10,
                            listvariable=eksamen, yscrollcommand=yScroll.set)
    eksamensliste.grid(row=0, column=1, rowspan=10,
    ...
    entEmnekode = Entry(window6, width=8, state="readonly",
                        textvariable=emnekode)
...
def resultatregistreringWindow8():
    ...
    def HentstudentnrOgLagEntries():
        ...
        def registrerResultater():
            ...
            for i in range(0, len(listeStudentnr), 1):
                ...
                sql = (
                    "UPDATE Eksamensresultat SET Karakter=%s WHERE Studentnr=%s AND Emnekode=%s AND Dato=%s AND Karakter IS NULL")
                data = (entryListe[i][1], entryListe[i]
                        [0], eksamenEmne, eksamenDato)
        ...
        henteSql = (
            "SELECT Studentnr FROM Eksamensresultat WHERE Emnekode=%s AND Dato=%s AND Karakter IS NULL")
        ...
        for i in range(0, len(listeStudentnr), 1):
            ...
            entStudnr = Entry(window8, width=6,
                              state="readonly", textvariable=studentnr)
    ...
    # knapper
    btnTilbake = Button(window8, text="Tilbake til hovedmeny",
...
def resultatredigeringWindow9():
    ...
    def hentResultatinfo():
        ...
        hentemarkor = mindatabase.cursor()
        sqlHente = (
            "SELECT Karakter FROM Eksamensresultat WHERE Studentnr=%s AND Emnekode=%s AND Dato=%s")
    ...
    def lagreInfo():
        ...
        sqlLagre = (
            "UPDATE Eksamensresultat SET Karakter=%s WHERE Studentnr=%s AND Emnekode=%s AND Dato=%s AND Karakter IS NOT NULL")
    ...
    btnTilbake = Button(window9, text="Tilbake til hovedmeny",
...
def resultatslettingWindow10():
    ...
    def hentInfo():
        ...
        hentemarkor = mindatabase.cursor()
        henteSql = (
            "SELECT * FROM Eksamensresultat WHERE Studentnr=%s AND Emnekode=%s and Dato=%s AND Karakter IS NULL")
    ...
    def sletteksamensresultat():
        ...
        slettemarkor = mindatabase.cursor()
        sletteSql = (
            "DELETE FROM Eksamensresultat WHERE Studentnr=%s AND Emnekode=%s AND Dato=%s AND Karakter IS NULL")
    ...
    # Knapper
    btnSlette = Button(
        window10, text="Slett eksamensregistrering", command=sletteksamensresultat)
    btnSlette.grid(row=12, column=5, padx=5, pady=5, sticky=E)
    btnTilbake = Button(
        window10, text="Tilbake til hovedmeny", command=window10.destroy)
...
def eksammensutksiftDatoWindow11():
    ...
    def hentInfo():
        ...
        for n in range(len(henteListe)):
            tekstboks.insert(
                "end", henteListe[n][0]+" "+str(henteListe[n][1])+" "+henteListe[n][2]+"\n")
    ...
    # Knapper
    btnTilbake = Button(
        window11, text="Tilbake til hovedmeny", command=window11.destroy)
...
def eksamensutskriftPeriodeWindow12():
    ...
    # Knapper
    btnTilbake = Button(
        window12, text="Tilbake til hovedmeny", command=window12.destroy)
...
def eksamensresultaterEmneWindow13():
    ...
    def hentInfo():
        ...
        henteSql = (
            "SELECT * FROM Eksamensresultat WHERE Emnekode=%s AND Dato=%s ORDER BY Studentnr")
        henteData = (henteEmnekode, henteDato)
        hentemarkor.execute(henteSql, henteData)
        ...
        for n in range(len(resultatliste)):
            txtEksamensresultater.insert(
                "end", resultatliste[n][0]+" "+resultatliste[n][1]+" "+resultatliste[n][2]+" "+resultatliste[n][3]+"\n")
    ...
    # Knapper
    btnTilbake = Button(
        window13, text="Tilbake til hovedmeny", command=window13.destroy)
...
def karakterstatestikkWindow14():
    ...
    def henteStatestikk():
        ...
        for i in range(len(statestikkliste)):
            txtEksamenstatestikk.insert("end", statestikkliste[i][0]+" "+statestikkliste[i][1]+" " +
                                        statestikkliste[i][2]+" "+statestikkliste[i][3]+" "+"Antall:"+" "+statestikkliste[i][4]+"\n")
    ...
    # Knapper
    btnTilbake = Button(
        window14, text="Tilbake til hovedmeny", command=window14.destroy)
    btnTilbake.grid(row=3, column=2, padx=5, pady=5, sticky=E)
...
def eksamensresultaterStudentWindow15():
    ...
    def henteResultater():
        ...
        for i in range(len(studentresultatliste)):
            txtStudentEksamensresultater.insert("end", studentresultatliste[i][0]+" "+studentresultatliste[i][1]+" " +
                                                studentresultatliste[i][2]+" "+studentresultatliste[i][3]+", Karakter: "+studentresultatliste[i][4]+"\n")
    ...
    txtStudentEksamensresultater.grid(
        row=2, column=1, padx=5, pady=5, sticky=W)

    # Knapper
    btnTilbake = Button(
        window15, text="Tilbake til hovedmeny", command=window15.destroy)
    btnTilbake.grid(row=3, column=2, padx=5, pady=5, sticky=E)
...
def vitnemålWindow16():
    ...
    def hentVitnemål():
        ...
        for i in range(len(vitnemalliste)):
            txtVitnemal.insert("end", vitnemalliste[i][0]+", "+vitnemalliste[i]
                               [1]+", "+vitnemalliste[i][2]+", Karakter: "+vitnemalliste[i][3]+"\n")
    ...
    entStudiepoeng = Entry(
        window16, width=4, state="readonly", textvariable=studiepoeng)
    entStudiepoeng.grid(row=3, column=1, padx=5, pady=5, sticky=W)

    # Knapper
    btnTilbake = Button(
        window16, text="Tilbake til hovedmeny", command=window16.destroy)
...
def eksamensoppmeldingWindow17():
    ...
    def lagreOppmelding():
        ...
        oppmeldingSql = (
            "INSERT INTO Eksamensresultat (Studentnr, Emnekode, Dato, Karakter) VALUES (%s,%s,%s,NULL)")
        ...
        try:
            ...
            txtTilbakemelding.insert(
                "end", "Student kan ikke meldes opp til eksamen")
    ...
    btnTilbake = Button(
        window17, text="Tilbake til hovedmeny", command=window17.destroy)
    btnTilbake.grid(row=4, column=2, padx=5, pady=5, sticky=E)
...
btnStudentoppretting = Button(
    window, text="Registrerer ny student", command=studentregistreringWindow2)
btnStudentoppretting.grid(row=1, column=0, padx=5, pady=5, sticky=NE)
btnStudentendring = Button(
    window, text="Rediger eksisterende student", command=studentredigeringWindow3)
btnStudentendring.grid(row=1, column=1, padx=5, pady=5, sticky=N)
btnStudentsletting = Button(
    window, text="Slett eksisterende student", command=studentslettingWindow4)
...
btnEksamensregistrering = Button(
    window, text="Registerer ny eksamen", command=eksamensregistreringWindow5)
btnEksamensregistrering.grid(row=3, column=0, padx=5, pady=5, sticky=NE)
btnEksamensredigering = Button(
    window, text="Rediger eksisterende eksamen", command=eksamensredigeringWindow6)
btnEksamensredigering.grid(row=3, column=1, padx=5, pady=5, sticky=N)
btnEksamenssletting = Button(
    window, text="Slett eksisterende eksamen", command=eksamensslettingWindow7)
...
btnResultatregistrering = Button(
    window, text="Registrer nye eksamensresultater", command=resultatregistreringWindow8)
btnResultatregistrering.grid(row=5, column=0, padx=5, pady=5, sticky=NE)
btnResultatredigering = Button(
    window, text="Rediger eksisterende eksamensresultat", command=resultatredigeringWindow9)
btnResultatredigering.grid(row=5, column=1, padx=5, pady=5, sticky=N)
btnResultatsletting = Button(
    window, text="Slett eksisterende eksamensresultat", command=resultatslettingWindow10)
btnResultatsletting.grid(row=5, column=2, padx=5, pady=5, sticky=NW)
btnEksamensoppmelding = Button(
    window, text="Meld opp studenter til eksamen", command=eksamensoppmeldingWindow17)
...
btnEksamenDato = Button(
    window, text="Eksamenssøk spesifik dato", command=eksammensutksiftDatoWindow11)
btnEksamenDato.grid(row=9, column=0, padx=5, pady=5, sticky=NE)
btnEksamenPeriode = Button(
    window, text="Eksamenssøk periode", command=eksamensutskriftPeriodeWindow12)
btnEksamenPeriode.grid(row=9, column=1, padx=5, pady=5, sticky=N)
btnResultaterEmne = Button(
    window, text="Eksamensresultater emne", command=eksamensresultaterEmneWindow13)
btnResultaterEmne.grid(row=9, column=2, padx=5, pady=5, sticky=NW)
btnResultaterStatestikk = Button(
    window, text="Karakterstatestikk", command=karakterstatestikkWindow14)
btnResultaterStatestikk.grid(row=10, column=0, padx=5, pady=5, sticky=NE)
btnStudentresultater = Button(
    window, text="Student eksamensresultater", command=eksamensresultaterStudentWindow15)
